...
<div class="sidebar" id="sidebar">
    ...
    <div class="menu-item {{ $isAuditori ? '' : 'locked' }}"
         @if($isAuditori) onclick="location.href='{{ route('berandaAuditori') }}'" @endif>
        <i class="bi bi-house-door"></i><span>Beranda</span>
    </div>

    <div class="menu-item {{ $isAuditori ? '' : 'locked' }}"
         @if($isAuditori) onclick="location.href='{{ route('petunjukAuditori') }}'" @endif>
...
<nav class="navbar custom-nav shadow-sm sticky-top px-3">
    ...
    <button class="btn btn-light me-2 rounded-circle shadow-sm {{ $isAuditori ? '' : 'locked' }}"
            id="toggleSidebar">
    ...
    <div class="ms-auto d-flex align-items-center gap-2">
        ...
        <button id="toggleTheme"
                class="btn btn-light rounded-circle shadow-sm {{ $isAuditori ? '' : 'locked' }}">
        ...
        <div class="dropdown">
            <button class="btn btn-light rounded-pill d-flex align-items-center px-2 py-1 shadow-sm
                           {{ $isAuditori ? '' : 'locked' }}"
                    type="button"
                    {{ $isAuditori ? 'data-bs-toggle=dropdown' : '' }}>
                <img src="{{ url('https://cdn-icons-png.flaticon.com/512/817/817787.png') }}"
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow">
                <li>
                    <a class="dropdown-item" href="{{ route('profileAuditori') }}">
...
<script>
    ...
    if (toggleSidebar && !toggleSidebar.classList.contains('locked')) {
