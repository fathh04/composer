...
<div class="container py-4">
    ...
    @php
        ...
        $canEksplore = $isVisual && $bolehEksplore;
    @endphp
    ...
    <div class="tab-wrapper">
        <ul class="nav nav-pills justify-content-between flex-wrap" id="kelasTab" role="tablist">
            ...
            <li class="nav-item flex-fill text-center" role="presentation">
                <button
                    class="nav-link w-100 {{ $isVisual ? '' : 'locked' }}"
                    {{ $isVisual ? 'data-bs-toggle=tab data-bs-target=#kuis' : '' }}
                    type="button" role="tab"
            ...
            <li class="nav-item flex-fill text-center" role="presentation">
                <button
                    class="nav-link w-100 {{ $isVisual ? '' : 'locked' }}"
                    {{ $isVisual ? 'data-bs-toggle=tab data-bs-target=#livecode' : '' }}
                    type="button" role="tab"
                    title="{{ $isVisual ? '' : 'Hanya untuk gaya belajar visual' }}"
                >
            ...
            <li class="nav-item flex-fill text-center" role="presentation">
                <button
                    class="nav-link w-100 {{ $isVisual ? '' : 'locked' }}"
                    {{ $isVisual ? 'data-bs-toggle=tab data-bs-target=#virtual' : '' }}
                    type="button" role="tab"
                    title="{{ $isVisual ? '' : 'Hanya untuk gaya belajar visual' }}"
                >
            ...
            <li class="nav-item flex-fill text-center" role="presentation">
                <button
                    class="nav-link w-100 {{ $isVisual ? '' : 'locked' }}"
                    {{ $isVisual ? 'data-bs-toggle=tab data-bs-target=#feed' : '' }}
                    type="button" role="tab"
                    title="{{ $isVisual ? '' : 'Hanya untuk gaya belajar visual' }}"
                >
            ...
            <li class="nav-item dropdown flex-fill text-center">
                <a
                    href="#"
                    class="nav-link w-100 dropdown-toggle {{ $isVisual ? '' : 'locked' }}"
                    id="eksploreBtn"
                    data-boleh="{{ $bolehEksplore ? '1' : '0' }}"
                    data-style="{{ $isVisual ? '1' : '0' }}"
